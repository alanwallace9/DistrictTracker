...
import { createRoomGroup } from '@/app/actions/daep/room-groups';
...
export function AddGroupDialog({ open, onOpenChange, onSuccess }: AddGroupDialogProps) {
  ...
  const onSubmit = async (data: CreateDAEPRoomGroupInput) => {
    setSubmitting(true);
    try {
      await createRoomGroup(data);
      toast({
        title: 'Success',
        description: `Room group "${data.group_name}" created successfully`,
      });
      reset();
      onOpenChange(false);
      onSuccess();
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to create room group',
        variant: 'destructive',
      });
    } finally {
      setSubmitting(false);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px] daep-theme">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a room group for separation logic. Rooms in the same group will be treated as one area.
          </DialogDescription>
        </DialogHeader>
